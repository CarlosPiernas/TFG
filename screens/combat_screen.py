...
ESTADO_IDLE       = 'idle'
ESTADO_ATACANDO   = 'attacking'
ESTADO_BLOQUEANDO = 'blocking'
...
class PantallaCombate(Screen):
    def __init__(self, **kwargs):
        ...
        self.barraVidaJugador = _BarraVida(
            color=COLOR_ANOMALIAS,
            size_hint=(1, 0.15)
        )
        ...
        self.barraVidaEnemigo = _BarraVida(
            color=COLOR_ANOMALIAS,
            size_hint=(1, 0.15)
        )
        ...
        self.botonCombate.bind(on_press=self.toggleCombate)
    ...
    def cargarCombate(self, nombreJugador, nombreEnemigo):
        self.nombreJugador = nombreJugador
        self.nombreEnemigo = nombreEnemigo
        self.imagenJugador.source = obtenerRutaSprite(nombreJugador, ESTADO_IDLE)
        self.imagenJugador.reload()
        self.imagenEnemigo.source = obtenerRutaSprite(nombreEnemigo, ESTADO_IDLE, esEnemigo=True)
        self.imagenEnemigo.reload()
        self.etiquetaTurno.text = 'TURNO 1'
        self.combateActivo = False
        self.botonCombate.text = 'PELEAR'
        self.botonCombate._bg_color = COLOR_GUARDIANES
        self.botonCombate.redibujarFondo()
        self.barraVidaJugador.actualizarVida(1.0)
        self.barraVidaEnemigo.actualizarVida(1.0)
        self.limpiarLog()

    def cambiarEstadoJugador(self, estado):
        self.imagenJugador.source = obtenerRutaSprite(self.nombreJugador, estado)
        self.imagenJugador.reload()

    def cambiarEstadoEnemigo(self, estado):
        self.imagenEnemigo.source = obtenerRutaSprite(self.nombreEnemigo, estado, esEnemigo=True)
        self.imagenEnemigo.reload()
    ...
    def actualizarTurno(self, numeroTurno):
        self.etiquetaTurno.text = f'TURNO {numeroTurno}'

    def actualizarVidaJugador(self, porcentaje):
        self.barraVidaJugador.actualizarVida(porcentaje)

    def actualizarVidaEnemigo(self, porcentaje):
        self.barraVidaEnemigo.actualizarVida(porcentaje)

    def limpiarLog(self):
        self.contenidoLog.clear_widgets()
    def toggleCombate(self, instance):
        self.combateActivo = not self.combateActivo
        if self.combateActivo:
            self.botonCombate.text = 'DETENER'
            self.botonCombate._bg_color = (0.7, 0.1, 0.1, 1)
            self.botonCombate.redibujarFondo()
